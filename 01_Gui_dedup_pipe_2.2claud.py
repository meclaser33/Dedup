# 01_Gui_dedup_pipe_2.2claud.py
# Mec_DB 통합 정리 시스템 v2.2
# ─────────────────────────────────────────────────────────────────────────────
# [v2.0 ~ v2.2 변경사항]
...
# [v2.2 추가 패치]
#   5. 체크포인트 파일명에 PC명(hostname) 포함 → 멀티PC 충돌 방지
#      예) _ckpt_MINIPC_b40b366b.json / _ckpt_SURFACE_xxxxxxxx.json
#   6. 결과 경로 Runs\Runs 이중 버그 수정 (BASE 경로 처리 정규화)
#   7. 탭 파라미터 1행 가로 배치
#   8. 탭 선택 시 제목 볼드 강조
#   9. 상태창에 스캔 시작 시각 표시
# ─────────────────────────────────────────────────────────────────────────────
...
# PC 식별자 — 체크포인트 파일명에 포함 (멀티PC 충돌 방지)
_HOSTNAME = socket.gethostname().upper()[:10]  # 최대 10자 대문자
...
def _save_ckpt(path: Path, data: dict):
    """
    체크포인트 저장.
    Google Drive 등 네트워크/동기화 폴더에서 atomic replace가 실패할 수 있으므로
    tmp → replace 실패 시 직접 쓰기로 폴백.
    """
    try:
        tmp = path.with_suffix(".tmp")
        tmp.write_text(json.dumps(data), encoding="utf-8")
        try:
            tmp.replace(path)   # atomic replace 시도
        except Exception:
            # Google Drive 등에서 replace 실패 → 직접 덮어쓰기
...
def _save_ckpt_both(local: Path, backup: Path, data: dict):
    """로컬(SCRIPT_DIR) 저장 후 BASE 백업도 시도"""
...
def run_folder_scan(root: str, base: str, depth_limit: int,
    ...
    # BASE 경로 정규화: 끝에 \Runs 가 붙어있으면 제거 (이중 Runs 방지)
    if base_path.name.lower() == "runs":
        base_path = base_path.parent

    min_bytes = min_dir_mb * 1024 * 1024

    # ── 체크포인트 파일명: PC명 + ROOT 해시 → 멀티PC 충돌 없음 ──────────
    ckpt_hash = hashlib.md5(str(root_path).encode()).hexdigest()[:8]
    ckpt_id   = f"{_HOSTNAME}_{ckpt_hash}"

    # 체크포인트 저장 위치: BASE(G:) 대신 스크립트 폴더 로컬에 저장 (Google Drive 저장 오류 방지)
    ckpt_path = CKPT_DIR / f"_ckpt_{ckpt_id}.json"
    ckpt_bak  = base_path / f"_ckpt_{ckpt_id}.json"   # BASE에도 백업 시도
    ...
    append_log(f"[FOLDER]   체크포인트 저장폴더: {CKPT_DIR}")
    append_log(f"[FOLDER]   체크포인트(로컬): {ckpt_path}")
    ...
    if not file_index:
        # 로컬에 없으면 BASE 백업에서 시도
        file_index = _load_ckpt(ckpt_bak)
        if file_index:
            append_log(f"    체크포인트 BASE 백업에서 로드 → {len(file_index):,}개")
    else:
    ...
    # 진단: file_index 중 현재 ROOT 하위 파일이 몇 개인지 확인
    root_str   = str(root_path)
    n_in_root  = sum(1 for k in file_index if k.startswith(root_str))
    ...
    run_dir    = base_path / "Runs" / f"run_{run_id}_fol"  # base_path 는 이미 Runs 제거됨
    ...
    append_log(f"[FOLDER] 체크포인트(로컬): {ckpt_path}")
    append_log(f"[FOLDER] 체크포인트(BASE): {ckpt_bak}")
...
# --- 탭 03: Folder ---
layout_tab_03 = [
    [sg.Text("내용물 유사 폴더 스캔 v2.2 (그룹핑)", key="-T03_TITLE-",
             font=_TAB_FONT_BOLD, text_color="cyan")],
...
while True:
    ...
    # ── 탭 전환 시 제목 볼드 강조 (현재 탭만 cyan, 나머지 회색) ──
    if event == "-TABS-":
        tab = values.get("-TABS-", "")
