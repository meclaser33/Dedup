# 01_Gui_dedup_pipe_2.3.py 서프고3 커밋 확인용 수정.
# Mec_DB 통합 정리 시스템 v2.3
# ─────────────────────────────────────────────────────────────────────────────
# [v2.3 변경사항 — 버그 수정]
#
...
def run_folder_scan(root: str, base: str, depth_limit: int,
    ...
    for cur_root, dirs, files in os.walk(root_path, topdown=True):
        ...
        # BUG FIX #2: 깊이 초과 시 이 폴더는 scan_dirs 에 추가하지 않음
        if depth >= depth_limit:
            dirs[:] = []   # 하위 탐색 차단
            continue        # scan_dirs·all_files 에 추가하지 않고 skip

        scan_dirs.add(cur)
        for fname in files:
            all_files.append(cur / fname)

        # 하위 탐색: depth+1 >= depth_limit 인 dirs 는 os.walk 가 들어오면
        # 위 조건에서 skip 되므로 dirs 수정 불필요
...
layout_tab_03 = [
    [sg.Text("내용물 유사 폴더 스캔 v2.3 (그룹핑)", key="-T03_TITLE-",
             font=_TAB_FONT_BOLD, text_color="cyan")],
